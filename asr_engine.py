...
class ASREngine:
    ...
    def recognize_with_speaker_diarization(self, audio_path, use_segment_recognition=True, session_dir=None):
        ...
        """
        self.logger.info(f"开始进行带说话人分离的音频识别: {audio_path}, 使用片段识别模式: {use_segment_recognition}")
        ...
        try:
            ...
            self.logger.info("音频加载成功")
            
            # 2. 使用WhisperX模型进行转录
            self.logger.info("开始使用WhisperX模型进行转录")
            result = self.model.transcribe(
                audio,
                language=self.language,
                batch_size=self.config.get_batch_size()
            )
            self.logger.info(f"转录完成，检测到语言: {result['language']}")
            
            # 保存转录结果
            import json
            transcription_path = os.path.join(session_dir, "transcription_result.json")
            with open(transcription_path, 'w', encoding='utf-8') as f:
                json.dump(result, f, ensure_ascii=False, indent=2)
            self.logger.info(f"转录结果已保存至: {transcription_path}")
            
            # 3. 使用WhisperX的对齐功能获取更精确的时间戳
            self.logger.info("开始进行音频对齐")
            ...
            # 保存对齐结果
            alignment_path = os.path.join(session_dir, "alignment_result.json")
            with open(alignment_path, 'w', encoding='utf-8') as f:
                json.dump(result, f, ensure_ascii=False, indent=2)
            self.logger.info(f"对齐结果已保存至: {alignment_path}")
            ...
            # 4. 使用pyannote/speaker-diarization-3.1模型进行说话人分离
            self.logger.info("开始进行说话人分离")
            try:
                ...
                rttm_path = os.path.join(session_dir, f"{base_name}.rttm")
                with open(rttm_path, "w") as f:
                    diarization.write_rttm(f)
                self.logger.info(f"RTTM结果已保存至: {rttm_path}")
                
                # 保存TXT结果
                txt_path = os.path.join(session_dir, f"{base_name}_diarization.txt")
                with open(txt_path, 'w', encoding='utf-8') as f:
                    for segment, _, speaker in diarization.itertracks(yield_label=True):
                        f.write(f"[{speaker}] 说话于 {segment.start:.1f}s - {segment.end:.1f}s\n")
                self.logger.info(f"文本结果已保存至: {txt_path}")
                
                # 创建diarize_segments格式，兼容whisperx.assign_word_speakers
                ...
                # 5. 将说话人信息分配给每个词
                self.logger.info("开始将说话人信息分配给每个词")
                result = whisperx.assign_word_speakers(diarize_segments, result)
                self.logger.info("说话人信息分配完成")
                
                # 保存最终结果
                final_result_path = os.path.join(session_dir, "final_result.json")
                with open(final_result_path, 'w', encoding='utf-8') as f:
                    json.dump(result, f, ensure_ascii=False, indent=2)
                self.logger.info(f"最终结果已保存至: {final_result_path}")
                
                # 如果使用片段识别模式，则根据说话人分离结果切分音频并单独识别
                if use_segment_recognition:
                    self.logger.info("使用片段识别模式，根据说话人分离结果切分音频并单独识别")
                    
                    # 导入片段识别模块
                    from speaker_segment_processor import SpeakerSegmentProcessor
                    
                    # 确保speaker_segments目录存在
                    speaker_segments_dir = os.path.join(session_dir, "speaker_segments")
                    os.makedirs(speaker_segments_dir, exist_ok=True)
                    self.logger.info(f"确保speaker_segments目录存在: {speaker_segments_dir}")
                    
                    # 创建说话人分段处理器并处理片段
                    processor = SpeakerSegmentProcessor(self)
                    segments = processor.process_segments(
                        audio_path,
                        diarize_segments_path,
                        session_dir
                    )
                    
                    self.logger.info(f"片段识别完成，共 {len(segments)} 个片段")
                    ...
                    # 保存最终处理结果为JSON格式
                    final_combined_path = os.path.join(session_dir, "final_combined_results.json")
                    with open(final_combined_path, 'w', encoding='utf-8') as f:
                        json.dump(segments, f, ensure_ascii=False, indent=2)
                    self.logger.info(f"最终组合结果已保存至: {final_combined_path}")
                    
                    # 生成话者分离摘要文本（优化格式）
                    combined_summary_path = os.path.join(session_dir, "combined_speaker_summary.txt")
                    ...
                    self.logger.info(f"组合模式话者分离摘要已保存至: {combined_summary_path}")
                    ...
                    for segment in segments:
                        enhanced_segment = {
                            ...
                            "segment_path": segment.get("segment_path", ""),  # 保留音频片段路径
                            "words": segment.get("words", [])  # 保留单词级别的时间戳
                        }
                ...
                self.logger.info("回退到简单的能量分离方法")
                # 如果pyannote的说话人分离失败，回退到简单的能量分离方法
                segments = []
                for segment in result["segments"]:
                    segments.append({
                        "text": segment["text"].strip(),
                ...
                self._simple_speaker_separation(segments, audio_path)
                return segments
            
            # 6. 处理结果，转换为标准格式
            segments = []
            for segment in result["segments"]:
                # 将SPEAKER_00转换为speakerA，SPEAKER_01转换为speakerB
                speaker = segment.get("speaker", "UNKNOWN")
                if speaker == "SPEAKER_00":
                    ...
                    speaker = f"speaker{chr(65 + (num % 26))}"
            ...
            # 保存最终处理结果
            final_segments_path = os.path.join(session_dir, "final_segments.json")
            with open(final_segments_path, 'w', encoding='utf-8') as f:
                json.dump(segments, f, ensure_ascii=False, indent=2)
            self.logger.info(f"最终处理结果已保存至: {final_segments_path}")
            
            # 生成话者分离摘要文本
            summary_path = os.path.join(session_dir, "speaker_summary.txt")
            ...
            self.logger.info(f"处理完成，共识别出 {len(segments)} 个片段")
            return segments
            
        except Exception as e:
            self.logger.error(f"带说话人分离的音频识别失败: {str(e)}")
            # 如果出现异常，回退到简单识别
            self.logger.info("回退到简单识别方法")
            segments = self.recognize(audio_path)
            self._simple_speaker_separation(segments, audio_path)
            return segments
    
    def recognize_with_alignment_segments(self, audio_path):
        """使用alignment_result.json拆分音频并进行分段识别
        ...
        Returns:
            list: 包含识别结果的列表，每个元素为一个字典，包含文本、开始时间、结束时间和说话人标识
        """
        self.logger.info(f"开始通过alignment_result.json拆分并识别音频: {audio_path}")
        ...
        try:
            ...
            # 4. 使用alignment_result.json拆分音频并单独识别
            self.logger.info("使用alignment_result.json拆分音频并单独识别")
            
            # 导入相关处理函数
            from speaker_segment_processor import process_alignment_segments
            
            # 处理片段
            segments = process_alignment_segments(
                self,
                audio_path,
                alignment_path,
                session_dir
            ...
            # 5. 使用pyannote/speaker-diarization为每个片段分配说话人
            self.logger.info("开始进行说话人分离，为每个片段分配说话人")
            try:
                from pyannote.audio import Pipeline
                from pyannote.core import Annotation
                ...
                for segment in segments:
                    ...
                    segment_mid = (segment_start + segment_end) / 2  # 取中点时间
                    
                    # 查找重叠的说话人片段
                    assigned_speaker = None
                    max_overlap = 0
                    
                    for diarize_segment in diarize_segments["segments"]:
                        ds_start = diarize_segment["start"]
                        ds_end = diarize_segment["end"]
                        
                        # 计算重叠时间
                        overlap_start = max(segment_start, ds_start)
                        overlap_end = min(segment_end, ds_end)
                        overlap = max(0, overlap_end - overlap_start)
                        
                        if overlap > max_overlap:
                            max_overlap = overlap
                            assigned_speaker = diarize_segment["speaker"]
                    ...
                    if not assigned_speaker:
                        for diarize_segment in diarize_segments["segments"]:
                            ...
                            # 如果segment的中点在diarize_segment的时间范围内，则分配该说话人
                            if ds_start <= segment_mid <= ds_end:
                                assigned_speaker = diarize_segment["speaker"]
                                break
                    ...
                    if assigned_speaker:
                        # 转换speaker标识为可读格式
                        readable_speaker = assigned_speaker
                        if assigned_speaker == "SPEAKER_00":
                            readable_speaker = "speakerA"
                        elif assigned_speaker == "SPEAKER_01":
                            readable_speaker = "speakerB"
                        elif assigned_speaker.startswith("SPEAKER_"):
                            # 提取数字部分并转换为字母
                            try:
                                num = int(assigned_speaker.split("_")[1])
                                readable_speaker = f"speaker{chr(65 + (num % 26))}"
                            except (ValueError, IndexError):
                                readable_speaker = assigned_speaker
                        
                        segment["speaker"] = readable_speaker
                        segment["original_speaker"] = assigned_speaker
                    else:
                        segment["speaker"] = "unknown"
                        segment["original_speaker"] = "unknown"
                ...
                self.logger.error(f"使用pyannote进行说话人分离失败: {str(e)}")
                self.logger.info("无法为每个片段分配说话人，使用默认标识")
                
                # 回退到简单的分配方法
                for i, segment in enumerate(segments):
                    segment["speaker"] = f"speaker{'A' if i % 2 == 0 else 'B'}"
                    segment["original_speaker"] = f"SPEAKER_{i % 2:02d}"
            
            # 保存最终处理结果为JSON格式
            final_combined_path = os.path.join(session_dir, "final_alignment_segments_results.json")
            with open(final_combined_path, 'w', encoding='utf-8') as f:
                json.dump(segments, f, ensure_ascii=False, indent=2)
            self.logger.info(f"最终组合结果已保存至: {final_combined_path}")
            
            # 生成片段摘要文本
            combined_summary_path = os.path.join(session_dir, "alignment_segments_summary.txt")
            with open(combined_summary_path, 'w', encoding='utf-8') as f:
                for segment in segments:
                    speaker = segment.get("speaker", "unknown")
                    f.write(f"[{speaker}] {segment['start_time']:.1f}s - {segment['end_time']:.1f}s:\n")
                    f.write(f"  原文: {segment.get('original_text', '')}\n")
                    f.write(f"  识别: {segment.get('text', '')}\n\n")
            self.logger.info(f"组合模式片段摘要已保存至: {combined_summary_path}")
            ...
            self.logger.error(f"通过alignment_result.json拆分音频处理失败: {str(e)}")
            # 回退到基本识别方法
